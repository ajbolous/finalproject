...
def serialAllocation(schedule, shovels, trucks, loaders, map):
    ...
    for shovel in shovels:
        t = DigTask(schedule.digLocation, startTime, ( startTime + timedelta(hours=9)), shovel.weightCapacity * 9*5,1)
        schedule.addTask(t)
    ...
    for loader in loaders:
        t = LoadTask(schedule.digLocation, startTime,  (startTime + timedelta(hours=9)), (loader.weightCapacity * 9*5))
    ...
    while True:
        ...
        for truck in trucks:
            ...
            noneSelected = False
            path, distance = map.calcShortestPath(schedule.digLocation, schedule.dumpLocations[1] )
            time = (2*distance/float(truck.speed))+1
            print distance, time
            t = HaulageTask(schedule.digLocation, schedule.dumpLocations[0], startTime, (startTime + timedelta(hours=time)), truck.weightCapacity)
            schedule.addTask(t)
            t.machine = truck
